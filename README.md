...
| `modules/companion_server.py` | ✅ | Telefon/masaüstü uyarlanır web arayüzü (stdlib, PWA) |
...
### Bağlanılan telefona göre arayüz geçişi (Companion)
JARVIS'in masaüstü penceresi fiziksel olarak telefona taşınamaz (ayrı işletim
sistemleri). Bunun yerine JARVIS, yerel ağda **cihaza göre kendini uyarlayan bir
web arayüzü** yayınlar (`companion_server.py`, ek paket gerekmez).

- `python main.py` çalışınca konsola bir adres yazılır (örn. `http://192.168.1.20:8770`).
  Telefon aynı Wi-Fi'da bu adresi tarayıcıdan açar (uzaktan için Tailscale).
- **"telefona bağlan"** → cihaz telefon olarak sınıflandırılır, arayüz **telefon
  düzenine** geçer. Hoparlör/kulaklık için ise ses o cihaza yönlendirilir (UI değişmez).
